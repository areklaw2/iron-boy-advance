...
use crate::{
    AluOperationsOpcode, ExecutionStrategy,
    arm::ArmInstruction,
...
impl TestBus {
    fn new(base_address: u32, opcode: u32, transactions: VecDeque<Transaction>) -> Self {
        TestBus {
            data: HashMap::new(),
            base_address,
            opcode,
...
pub fn run_single_step_tests<S: ExecutionStrategy + Default + Sync>() {
    let files_to_run = ["arm_b_bl.json.bin", "arm_bx.json.bin"];
